...
public class CreateNFTWindow : WindowBase
{
    ...
    async void Awake()
    {
        this.DeployButton.onClick.AddListener(async delegate
        {
            ...
            if (receipt.Success)
                NFTWallet.Instance.RegisterDeployedNFT(nftName, symbol, ownerOnlyMinting, receipt.NewContractAddress, NFTWallet.Instance.StratisUnityManager.GetAddress().ToString());
        });
    }
}
